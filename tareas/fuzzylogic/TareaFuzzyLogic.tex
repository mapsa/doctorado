...
\title{\textbf{Tarea Fuzzy Logic} \\Inferencia de Conocimiento en Sistemas de Información\\ Profesor: Dr.
Héctor Allende}
...
\subsection{Operadores s-norma} \textit{Verificar que los operadores POR (s-norm probabilistic) y LOR (s-norm de Lukasiewics) cumplen las propiedades de s-norma.}
...
\section{Pregunta 2}
\textit{Verificar si las siguientes propiedades se cumplen para Fuzzy sets. Use las t-norm y s-norm de problema
...
\item{Usando LAND y LOR}\newline
$LOR(A,LAND(B,C))=min(A+max(B+C-1,0),1)$\newline
$LOR(A,B)*LOR(A,C)=max(min(A+B,1)+min(A+C,1)-1,0)$\newline
Las funciones no son iguales, basta tomar el caso donde $B+C <1 \wedge A+B >1 \wedge A+C > 1$ se tendría que:
\newline
$LOR(A,LAND(B,C))=min(A,1)=A$
$LOR(A,B)*LOR(A,C)=max(1+1-1,0)=1$
\newline
$\therefore$ no son iguales
\end{enumerate}
...
\section{Pregunta 3}
\textit{Sean R y S relaciones Fuzzy de X en Y. Verificar que las relaciones Fuzzy cumplen las siguientes propiedades. Use las t-norm y s-norm de Mamdani.}

\begin{enumerate}
\item {\color{red}$(R \cup S)^{-1} = R^{-1} \cup S^{-1}$}
\newline
Se define la unión como sigue:
...
La expresión discreta del problema es usando la forma matricial de $R$ y $S$ que se definen como:
...
$$\mu_{(\overline{R})^{-1}}(y,x)=\mu_{\overline{R}}(x,y)=1-\mu_R(x,y)$$
 ...
 La expresión discreta del problema puede ser representada por:
...
\section{Pregunta 4}
\textit{Sean $A$,$A'$ Fuzzy sets y sean $R$, $R'$ relaciones Fuzzy de X en Y y S,T relaciones Fuzzy de Y en Z. Verificar que:}

\begin{itemize}
\item {\color{red}$(R \circ S) ^{-1} = S^{-1} \circ R^{-1} $}

Daro que la relación $R \circ S$ es una relación de $X$ en $Z$, la relación inversa $(R \circ S) ^{-1}$ es una relación que va de $Z$ en $X$.

Se define la función de pertenencia de la relación $(R \circ S) ^{-1}$ como:
...
Por otra parte la relación $S^{-1} \circ R^{-1}$ se define como:
...
Que corresponde a la equivalencia mostrada en la ecuación \ref{eq:41}. $\therefore$ se cumple.


\item {\color{red}$(R \circ S) \neq S \circ R$}

La desigualdad se cumple ya que la relación $R \circ S$ está definida en $X \times Z$ y la relación $S \circ R$ ni siquiera es válida, debido a que la relación $S$ está definida en $Y \times Z$ y la relación $R$ en $X \times Y$ ($Z$ y $X$ son distintos). 

\item {\color{red}$S \subseteq T \Rightarrow R \circ S \subseteq  R \circ T$}

Se tiene que la relación de inclusión entre $S$ y $T$, $S \subseteq T$ se define como:

$$\mu_S(y,z) \leq \mu_T(y,z) \quad \forall x \in X \quad \forall y \in Y$$

Por lo tanto, debe demostrarse que se cumple lo siguiente:

$$\mu_S(y,z) \leq \mu_T(y,z) \Rightarrow max_{y \in Y}[min(\mu_{R}(x,y),\mu_{S}(y,x))] \leq max_{y \in Y}[min(\mu_{R}(x,y),\mu_{T}(y,x))]$$ 


La expresión al lado derecho de la implicancia puede expresarse 

 $=\left\{
 \begin{array}{ll}
  max_{y \in Y}[\mu_{S}(y,x)] \leq max_{y \in Y}[\mu_{T}(y,x)]  & \mbox{if } \mu_{R}(x,y) > \mu_{S}(y,x) \wedge \mu_{R}(x,y) > \mu_{T}(y,x)\\
  max_{y \in Y}[\mu_{S}(y,x)] \leq max_{y \in Y}[\mu_{R}(x,y)]  & \mbox{if } \mu_{R}(x,y) > \mu_{S}(y,x) \wedge \mu_{R}(x,y) \leq \mu_{T}(y,x)\\
  max_{y \in Y}[\mu_{R}(x,y)] \leq max_{y \in Y}[\mu_{T}(y,x)]  & \mbox{if } \mu_{R}(x,y) \leq \mu_{S}(y,x) \wedge \mu_{R}(x,y) > \mu_{T}(y,x)\\
  max_{y \in Y}[\mu_{R}(x,y)] \leq max_{y \in Y}[\mu_{R}(x,y)]  & \mbox{if } \mu_{R}(x,y) \leq \mu_{S}(y,x) \wedge \mu_{R}(x,y) \leq \mu_{T}(y,x)\\       \end{array}
\right.$

Si $\mu_S(y,z) \leq \mu_T(y,z)$ podemos reescribir lo anterior como:

 $=\left\{
 \begin{array}{ll}
  max_{y \in Y}[\mu_{S}(y,x)] \leq max_{y \in Y}[\mu_{T}(y,x)]  & \mbox{if } \mu_S(y,z) \leq \mu_T(y,z) \leq \mu_{R}(y,x) \quad \forall x,\forall y \forall z\\
  max_{y \in Y}[\mu_{S}(y,x)] \leq max_{y \in Y}[\mu_{R}(x,y)]  & \mbox{if } \mu_S(y,z) \leq \mu_{R}(y,x) \leq \mu_T(y,z) \quad \forall x,\forall y \forall z\\
  max_{y \in Y}[\mu_{R}(x,y)] \leq max_{y \in Y}[\mu_{T}(y,x)]  & \mbox{if } \mu_{R}(y,x) \leq \mu_S(y,z) \leq \mu_T(y,z) \quad \forall x,\forall y \forall z\\
\end{array}
\right.$

En el primer caso, dado que $\mu_S(y,z) \leq \mu_T(y,z) \quad \forall y, \forall z$ implica que $max_{y \in Y}[\mu_{S}(y,x)] \leq max_{y \in Y}[\mu_{T}(y,x)]$. Los otros dos casos son análogos.

Es claro ver que todas las implicancias son verdaderas cuando se cumple que $\mu_S(y,z) \leq \mu_T(y,z)$.
